...
public:
  ...
  void auto_resize( bool t ) { _auto_resize = t; }
    ...
    void column_labels( const char** c ) { _column_labels = c; }
    void column_widths( const int* w );

    const char** column_labels() const { return _column_labels; }
    const int* column_widths() const { return _column_widths; }

protected:

    int absolute_item_index( const Fl_Widget* g );
...
protected:
    ...
    int       _value;
    const char** _column_labels;
    const int*   _column_widths;
