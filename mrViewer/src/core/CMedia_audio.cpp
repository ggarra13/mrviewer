...
bool CMedia::seek_to_position( const boost::int64_t frame, const int flags )
{
  ...
  // Make sure we are at that frame
  AVCodecContext *ctx = stream->codec;
		      ...
		      const boost::int64_t frame, const AVPacket& pkt )
{
  ...
  for (;;)
	...
	{
	  std::cerr << "A  bpf: " << bytes_per_frame << std::endl
		    << "  used: " << _audio_buf_used << std::endl;
	}
#endif
				     ...
				     const bool is_seek )
{
  ...
  assert( _audio_last_frame <= last_frame() );

  while ( !_audio_packets.is_seek() && !_audio_packets.empty() )
    {
      AVPacket& pkt = _audio_packets.front();
...
{ 
  ...
  while ( got_audio != kDecodeOK && !_audio_packets.empty() )
	...
	{
	  clear_stores();  // audio stores MUST be cleared when cleared
  ...
  if ( stopped() )
    {
      if ( has_audio() )
	{
	   std::cerr << "decode audio " << _seek_frame << std::endl;
