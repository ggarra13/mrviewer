                             ...
                             const AVPacket& pkt )
{
...

  {
      ...
      av_assert0( audio_size <= AVCODEC_MAX_AUDIO_FRAME_SIZE );
      ...
      av_assert0( audio_size > 0 );
      av_assert0( audio_size + _audio_buf_used <= _audio_max );
                                ...
                                const bool detail)
{
  ...
  std::cerr << this << std::dec << " " << name()
            << " S:" << frame << " D:" << _adts
            << " A:" << _audio_frame << " " << routine << " audio stores #"
            << _audio.size() << ": ";
