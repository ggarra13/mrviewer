 ...
 * 
 * @brief  Pixel operations using NVidia's half data types.
 * 
 * 
 */
...
namespace mrv {
  ...
  ImagePixel VideoFrame::pixel_h16( const unsigned int x, 
				    const unsigned int y ) const
  {   
    ImagePixel p( 0.f, 0.f, 0.f, 0.f );
    unsigned offset = y * _width + x;

    half* d = (half*) _data.get();
    half* col = d + offset * _channels;
    half yp = 0.f, cb = 0.f, cr = 0.f;
    switch( _format )
      {
      case kLummaA:
	{
  ...
  void VideoFrame::pixel_h16( const unsigned int x, const unsigned int y,
			      const ImagePixel& p )
  {
    unsigned offset = y * _width + x;
