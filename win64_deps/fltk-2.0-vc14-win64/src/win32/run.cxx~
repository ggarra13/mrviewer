  ...
  case WM_GETMINMAXINFO:
	  if (lParam)
	  {
		  LPMINMAXINFO lp = (LPMINMAXINFO)lParam;
                  ...
                  POINT p; p.x = p.y = 0;
                  HMONITOR hMonitor = MonitorFromPoint(p, 0);
                  GetMonitorInfo( hMonitor, &screen );
                  // Avoid maximize to cover taskbar
                  // and add the size of the border which is gone
                  const int kBorder = 12;
                  // std::cerr << "1 pos " << lp->ptMaxPosition.x << ", "
                  //           << lp->ptMaxPosition.y << std::endl;
                  // std::cerr << "1 size " << lp->ptMaxSize.x << ", "
                  //           << lp->ptMaxSize.y << std::endl;
                  // std::cerr << "1 metrics " << lp->ptMaxTrackSize.x << ", "
                  //           << lp->ptMaxTrackSize.y << std::endl;
                  // std::cerr << "1 rcWork " << screen.rcWork.left << ", "
                  //           << screen.rcWork.top << std::endl;
                  lp->ptMaxPosition.x += screen.rcWork.left;
                  lp->ptMaxPosition.y += screen.rcWork.top;
                  lp->ptMaxSize.x = screen.rcWork.right - screen.rcWork.left +
                                    kBorder;
                  lp->ptMaxSize.y = screen.rcWork.bottom - screen.rcWork.top +
                                    kBorder;
                  int x = GetSystemMetrics(SM_CXMAXTRACK);
                  int y = GetSystemMetrics(SM_CYMAXTRACK);
                  lp->ptMaxTrackSize.x = x;
                  lp->ptMaxTrackSize.y = y;
                  // std::cerr << "2 pos " << lp->ptMaxPosition.x << ", "
                  //           << lp->ptMaxPosition.y << std::endl;
                  // std::cerr << "2 size " << lp->ptMaxSize.x << ", "
                  //           << lp->ptMaxSize.y << std::endl;
                  // std::cerr << "2 metrics " << lp->ptMaxTrackSize.x << ", "
                  //           << lp->ptMaxTrackSize.y << std::endl;
	  }
...
void Window::borders( fltk::Rectangle *r ) const
{
  ...
  if (shown()) {
    ...
    if (resizable() || maxw != minw || maxh != minh) // resizable
      style = WS_SYSMENU | WS_THICKFRAME | WS_MAXIMIZEBOX | WS_CAPTION;
    else
...
#if 0
  // The older api, which apparently does not return correct sizes on XP or
  // win64?
